...
def model(images, outputs = 6, weight_decay=1e-5, is_training=True):
    ...
    #F = C(P2,P3,P4,P5)
    F = tf.concat(P_concat, axis=-1) #不同尺度的特征图进行concat
    ...
    with tf.variable_scope('feature_results'):
        ...
        with slim.arg_scope([slim.conv2d],
                            weights_regularizer=slim.l2_regularizer(weight_decay),
                            activation_fn=None):
            S = slim.conv2d(F, outputs, 1) #进行1X1卷积处理

    seg_S_pred = tf.nn.sigmoid(S) #经过激活函数输出预测结果
...
def loss(y_true_cls, y_pred_cls,
    ...
    _, Lc = dice_coefficient(Gn[5], Sn[5], training_mask=training_mask) #整个文本实例的loss
