    ...
    for i in range(len(scale_ratio)):
        ...
        for poly_idx, poly_tag in enumerate(zip(polys, tags)):
            ...
            if poly_idx not in ignore_poly_mark:
                shrinked_polys = shrink_poly(poly.copy(), scale_ratio[i])#收缩区域坐标计算
        ...
        for i in index:
            try:
                ...
                if text_polys.shape[0] == 0:
                    continue
                text_polys, text_tags = check_and_validate_polys(text_polys, text_tags, (h, w)) #校验
                ...
                if np.random.rand() < background_ratio:
                    # crop background
                    im, text_polys, text_tags = crop_area(im, text_polys, text_tags, crop_background=True) #随机裁剪
                    if text_polys.shape[0] > 0:
                    ...
                    im, text_polys, text_tags = crop_area(im, text_polys, text_tags, crop_background=False) #随机裁剪
                    if text_polys.shape[0] == 0:
                        continue
                    # h, w, _ = im.shape
                ...
                seg_maps.append(seg_map_per_image[::4, ::4, :].astype(np.float32)) #1/4图
                training_masks.append(training_mask[::4, ::4, np.newaxis].astype(np.float32)) #1/4图
