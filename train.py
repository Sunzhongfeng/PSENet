...
def average_gradients(tower_grads):
    average_grads = [] #创建平均梯度列表
    for grad_and_vars in zip(*tower_grads):#双层列表转置
        grads = []
        for g, _ in grad_and_vars:
            expanded_g = tf.expand_dims(g, 0)#给梯度添加冗余维度0
            grads.append(expanded_g)#梯度存入列表grads

        grad = tf.concat(grads, 0)#维度0上合并
        grad = tf.reduce_mean(grad, 0)#维度0平均
...
def main(argv=None):
    ...
    for i, gpu_id in enumerate(gpus):
        with tf.device('/gpu:%d' % gpu_id):#限定使用哪块GPU
            with tf.name_scope('model_%d' % gpu_id) as scope:#命名空间
                ...
                grads = opt.compute_gradients(total_loss)#计算单个GPU梯度
                tower_grads.append(grads)#添加到梯度列表tower_grads

    grads = average_gradients(tower_grads)#计算平均梯度
    apply_gradient_op = opt.apply_gradients(grads, global_step=global_step)#更新模型参数
    ...
    init = tf.global_variables_initializer()#初始化全部参数
